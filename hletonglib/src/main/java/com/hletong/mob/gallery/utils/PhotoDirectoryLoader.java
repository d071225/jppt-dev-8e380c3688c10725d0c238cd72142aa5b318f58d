...
public class PhotoDirectoryLoader extends CursorLoader {
    ...
    public PhotoDirectoryLoader(Context context, boolean showGif, int size) {
        super(context);

        setProjection(IMAGE_PROJECTION);
        setUri(Media.EXTERNAL_CONTENT_URI);
        setSortOrder(Media.DATE_ADDED + " DESC");
        String selection = "(" + MIME_TYPE + "=? or " + MIME_TYPE + "=? or " + MIME_TYPE + "=? " + (showGif ? ("or " + MIME_TYPE + "=?") : "") + " ) and " + Media.SIZE + ">=" + size;
        setSelection(selection);
        //Log.e("print", selection);//Media.SIZE + ">=30720"
