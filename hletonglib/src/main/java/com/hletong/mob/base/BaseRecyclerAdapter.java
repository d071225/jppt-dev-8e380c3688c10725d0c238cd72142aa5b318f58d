...
public abstract class BaseRecyclerAdapter<T> extends RecyclerView.Adapter<BaseHolder<T>> {
    ...
    public BaseRecyclerAdapter(Context context, @Nullable List<T> data) {
        if (data == null) {
            data = new ArrayList<>();
        }
        this.mData = data;
    ...
    public void addData(List<T> loadMore) {
        if (loadMore != null && loadMore.size() > 0) {
            ...
            notifyItemRangeInserted(getBeginIndex() + mData.size() - loadMore.size(), loadMore.size());
        }
    }
    ...
    public void addData(int position, List<T> data) {
        if (data != null && data.size() > 0) {
            ...
            notifyItemInserted(getBeginIndex() + position);
        }
    }
    ...
    public void add(T data) {
        if (data != null) {
            mData.add(data);
            notifyItemInserted(getBeginIndex() + mData.size()-1);
        }
    }
    ...
    public void remove(int position) {
        if (position >= 0 && position < mData.size()) {
            ...
            notifyItemRemoved(getBeginIndex() + position);
        }
    }
    ...
    public int getItemCount() {
        return getBeginIndex() + getDataCount();
    }
